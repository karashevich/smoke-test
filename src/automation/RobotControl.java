...
import automation.ui.ClickRipple;
...
public class RobotControl {
  ...
  @Nullable
  public JTextField findJTextField(String textComponent){
  ...
  public Point findFrameworkCheckbox(String frameworkId){
    final Component component = GuiUtil.findComponentsByType(FrameworksTree.class, IdeFocusManager.getGlobalInstance().getFocusOwner()).get(0);
    FrameworksTree frameworksTree = (FrameworksTree) component;

//    FrameworkSupportNode node = (FrameworkSupportNode) GuiUtil.traverse((TreeNode) model.getRoot(), new GuiUtil.Acceptor() {
//      @Override
//      public boolean accept(TreeNode treeNode) {
//        if (treeNode instanceof FrameworkSupportNode && ((FrameworkSupportNode)treeNode).getId().equals("hibernate"))
//          return true;
//        return false;
//      }
//    });
//    assert node != null;
////    final FrameworkSupportNode arq = (FrameworkSupportNode) child.getChildren().get(0);
//    final TreeNode[] path = node.getPath();
//    int[] indexes = new int[path.length - 1];
//    for (int i = 1; i < path.length; i++) {
//      final int index = path[i].getParent().getIndex(path[i]);
//      indexes[i - 1] = index;
//    }
    for (int i = 0; i < frameworksTree.getRowCount(); i++) {
      final TreePath pathForRow = frameworksTree.getPathForRow(i);
      final FrameworkSupportNodeBase nodeBase = (FrameworkSupportNodeBase) pathForRow.getLastPathComponent();
      if (nodeBase.getId().equals(frameworkId)) {
        final Rectangle checkboxBounds = ((CheckboxTreeBase.CheckboxTreeCellRendererBase)frameworksTree.getCellRenderer()).myCheckbox.getBounds();
        final Rectangle pathBounds = frameworksTree.getPathBounds(pathForRow);
        final Point locationOnScreen = frameworksTree.getLocationOnScreen();
        return new Point(locationOnScreen.x + pathBounds.x + checkboxBounds.x + checkboxBounds.width / 2, locationOnScreen.y + pathBounds.y + checkboxBounds.y + checkboxBounds.height / 2);
      }
    ...
    public void waitUi(final Runnable runnable, final String componentText, int timeout) throws Exception {
      EdtInvocationManager.getInstance().invokeLater(new Runnable() {
        ...
        public void run() {
          ...
          if (GuiUtil.findComponentByText(componentText, IdeFocusManager.getGlobalInstance().getFocusOwner()) != null) {
            runnable.run();
          } else {
            try {
              queueOfTasks.put(new Computable<Runnable>() {
